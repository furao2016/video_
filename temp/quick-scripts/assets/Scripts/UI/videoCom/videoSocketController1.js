(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Scripts/UI/videoCom/videoSocketController1.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '4b3801LxMZGK6M0kM8Tq7LA', 'videoSocketController1', __filename);
...
(function (baccarat) {
  ...
  var onOpenCallback = function onOpenCallback(event) {
  ...
  var onCloseCallback = function onCloseCallback(event) {
  ...
  var onMessageCallback = function onMessageCallback(event) {
    ...
    switch (header.type) {
      case WEBSOCKET_HEADER_TYPE_VIDEO:
        //使用队列方式处理
        //H264
        if (messageData[0] === 0 && messageData[1] === 0 && messageData[2] === 0 && messageData[3] === 1) {
          if (this._lastBuffer) {
          ...
          var newBuffer = new Uint8Array(this._lastBuffer.length + messageData.length);
  ...
  proto.decodeFromQueueTimer = function () {
    ...
    if (selPkg) {
      this._decoder.decode(selPkg.buffer);
      var audioPkg = cache.getDecodeAACBuffer();
      if (audioPkg) this.audioPlayer.asset.source.add(audioPkg.buffer);
    }
  };

  proto._setupWorker = function () {
    var self = this;
    var onPictureDecoded = function onPictureDecoded(buffer, format, width, height, infos) {
      if (self.onPictureDecoded) self.onPictureDecoded(buffer, format, width, height, infos);
    };
    ...
    if (this._useWorker) {
      ...
      worker.addEventListener("message", function (e) {
        ...
        onPictureDecoded.call(self, new Uint8Array(data.buf, 0, data.length), 0, data.width, data.height, data.infos);
      }, false);
      ...
      this._decodeVideo = function (ts, videoData, parInfo) {
        ...
        if (ts === 0) this._decoder.decode(videoData);else this._packageCache.pushH264(ts, videoData);
      };
    }
  };
  /**
  ...
  h264CacheProto.getDecodeH264Buffer = function (dt) {
    this._deltaTime += dt;
    if (this._deltaTime < this._currentFPS) return null;
  ...
  h264CacheProto._searchVideoKeyframe = function () {
    var videoCache = this._videoCache;
    if (videoCache.length < SKIP_KEYFRAMES) return null;

    var selIdx = -1;
    for (var i = 0; i < videoCache.length; i++) {
      if (videoCache[i].keyframe) selIdx = i;
    }
    if (selIdx !== -1) {
  ...
  h264CacheProto.pushH264 = function (timestamp, buffer) {
    if (!this.getPackageByTimestamp(this._videoCache, timestamp)) {
      if (this.receivedKeyframe) {
        if (timestamp > this.lastPlayedTimestamp) this._videoCache.push(new baccarat.H264Package(timestamp, buffer));else {
          if (this.lastPlayedTimestamp - timestamp > 2000) this.lastPlayedTimestamp = 0; //reset, because timestamp has been reset.
  ...
  h264CacheProto.getDecodeAACBuffer = function () {
    ...
    if (audioCache.length === 0) return null;
  ...
  h264CacheProto.skipAACBuffer = function (timestamp) {
    ...
    if (idx > 0) audioCache.splice(0, idx);
  };
...
cc._RF.pop();
        ...
        //# sourceMappingURL=videoSocketController1.js.map
