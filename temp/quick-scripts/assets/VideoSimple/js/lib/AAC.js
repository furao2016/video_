(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/VideoSimple/js/lib/AAC.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '4da83ePCYdD9aA09Lcd0fys', 'AAC', __filename);
// VideoSimple/js/lib/AAC.js
    ...
    (function (global) {
        ...
        var EventEmitter = AV.BufferList = function (_super) {
            ...
            EventEmitter.prototype.once = function (event, fn) {
                var _cb;
                return this.on(event, _cb = function cb() {
                    this.off(event, _cb);
        ...
        var Stream = AV.Stream = function () {
            ...
            float64Fallback = function float64Fallback() {
            ...
            decodeString = function decodeString(offset, length, encoding, advance) {
        ...
        var MozillaAudioDevice = AV.MozillaAudioDevice = function (_super) {
            ...
            createTimer = function createTimer(fn, interval) {
                var url, worker;
            ...
            destroyTimer = function destroyTimer(timer) {
                if (timer.terminate) {
                    timer.terminate();
        ...
        var Asset = AV.Asset = function (_super) {
            ...
            Asset.prototype.decodeToBuffer = function (callback) {
                ...
                length = 0;
                chunks = [];
                this.on('data', dataHandler = function dataHandler(chunk) {
                    length += chunk.length;
        ...
        var BufferSource = AV.BufferSource = function (_super) {
            ...
            BufferSource.prototype.add = function (data) {
                //David's added.
    ...
    (function (global) {
        ...
        var ADTSDemuxer = AV.Demuxer.extend(function () {
            ...
            this.readHeader = function (stream) {
                if (stream.read(12) !== 0xfff) throw new Error('Invalid ADTS header.');
        ...
        var CCEElement = function () {
            ...
            var CCE_SCALE = new Float32Array([1.09050773266525765921, 1.18920711500272106672, 1.4142135623730950488016887, 2.0]);

            CCEElement.prototype = {
                decode: function decode(stream, config) {
                ...
                applyIndependentCoupling: function applyIndependentCoupling(index, data) {
                ...
                applyDependentCoupling: function applyDependentCoupling(index, data) {
        ...
        var AACDecoder = AV.Decoder.extend(function () {
            ...
            this.prototype.setCookie = function (buffer) {
                ...
                if (this.config.profile === AOT_ESCAPE) this.config.profile = 32 + stream.read(6);
            ...
            this.prototype.processSingle = function (element, channel) {
                ...
                if (profile === AOT_AAC_MAIN) throw new Error("Main prediction unimplemented");

                if (profile === AOT_AAC_LTP) throw new Error("LTP prediction unimplemented");
                ...
                if (element.gainPresent) throw new Error("Gain control not implemented");
            ...
            this.prototype.processPair = function (element, channel) {
                ...
                // Mid-side stereo
                if (element.commonWindow && element.maskPresent) this.processMS(element, l_data, r_data);
                ...
                if (left.gainPresent) throw new Error("Gain control not implemented");

                if (right.gainPresent) throw new Error("Gain control not implemented");
                    ...
                    for (var i = 0; i < maxSFB;) {
                        ...
                        if (bandTypes[idx] === ICStream.INTENSITY_BT || bandTypes[idx] === ICStream.INTENSITY_BT2) {
                            for (; i < end; i++, idx++) {
                                ...
                                if (element.maskPresent) c *= element.ms_used[idx] ? -1 : 1;
                ...
                switch (info.windowSequence) {
                    ...
                    case ICStream.EIGHT_SHORT_SEQUENCE:
                        ...
                        for (var i = 0; i < shortLen; i++) {
                            ...
                            if (i < trans) output[mid + 4 * shortLen + i] = overlap[mid + shortLen * 4 + i] + buf[shortLen * 7 + i] * shortWindows[shortLen - 1 - i] + buf[shortLen * 8 + i] * shortWindows[i];
                        }

                        // window the second half and save as overlap for next frame
                        for (var i = 0; i < shortLen; i++) {
                            if (i >= trans) overlap[mid + 4 * shortLen + i - length] = buf[shortLen * 7 + i] * shortWindows[shortLen - 1 - i] + buf[shortLen * 8 + i] * shortWindows[i];
        ...
        var Huffman = function () {
            ...
            return {
                findOffset: function findOffset(stream, table) {
                    var off = 0,
                        len = table[off][0],
                        cw = stream.read(len);
                ...
                signValues: function signValues(stream, data, off, len) {
                    for (var i = off; i < off + len; i++) {
                        if (data[i] && stream.read(1)) data[i] = -data[i];
                    }
                },

                getEscape: function getEscape(stream, s) {
                    var i = 4;
                    while (stream.read(1)) {
                        i++;
                ...
                decodeScaleFactor: function decodeScaleFactor(stream) {
                    var offset = this.findOffset(stream, HCB_SF);
                    return HCB_SF[offset][2];
                },

                decodeSpectralData: function decodeSpectralData(stream, cb, data, off) {
                    ...
                    // sign and escape
                    if (cb < 11) {
                        if (UNSIGNED[cb - 1]) this.signValues(stream, data, off, cb < 5 ? QUAD_LEN : PAIR_LEN);
                    } else if (cb === 11 || cb > 15) {
                        this.signValues(stream, data, off, cb < 5 ? QUAD_LEN : PAIR_LEN);

                        if (Math.abs(data[off]) === 16) data[off] = this.getEscape(stream, data[off]);

                        if (Math.abs(data[off + 1]) === 16) data[off + 1] = this.getEscape(stream, data[off + 1]);
                    } else {
        ...
        var ICStream = function () {
            ...
            ICStream.prototype = {
                decode: function decode(stream, config, commonWindow) {
                    ...
                    if (this.pulsePresent = stream.read(1)) {
                        if (this.info.windowSequence === ICStream.EIGHT_SHORT_SEQUENCE) throw new Error("Pulse tool not allowed in eight short sequence.");
                ...
                decodeBandTypes: function decodeBandTypes(stream, config) {
                    ...
                    for (var g = 0; g < groupCount; g++) {
                        ...
                        while (k < maxSFB) {
                            ...
                            if (end > maxSFB) throw new Error("Too many bands (" + end + " > " + maxSFB + ")");
                ...
                decodeScaleFactors: function decodeScaleFactors(stream) {
                    var groupCount = this.info.groupCount,
                        maxSFB = this.info.maxSFB,
                        offset = [this.globalGain, this.globalGain - 90, 0],
                        // spectrum, noise, intensity
                    idx = 0,
                        noiseFlag = true,
                            ...
                            switch (bandTypes[idx]) {
                                ...
                                default:
                                    for (; i < runEnd; i++, idx++) {
                                        ...
                                        if (offset[0] > 255) throw new Error("Scalefactor out of range: " + offset[0]);
                ...
                decodePulseData: function decodePulseData(stream) {
                    var pulseCount = stream.read(2) + 1,
                        pulseSWB = stream.read(6);

                    if (pulseSWB >= this.info.swbCount) throw new Error("Pulse SWB out of range: " + pulseSWB);
                    ...
                    if (this.pulseOffset[0] > 1023) throw new Error("Pulse offset out of range: " + this.pulseOffset[0]);

                    for (var i = 1; i < pulseCount; i++) {
                        this.pulseOffset[i] = stream.read(5) + this.pulseOffset[i - 1];
                        if (this.pulseOffset[i] > 1023) throw new Error("Pulse offset out of range: " + this.pulseOffset[i]);

                        this.pulseAmp[i] = stream.read(4);
                    }
                },

                decodeSpectralData: function decodeSpectralData(stream) {
                    var data = this.data,
                        info = this.info,
                        maxSFB = info.maxSFB,
                        windowGroups = info.groupCount,
            ...
            ICSInfo.prototype = {
                decode: function decode(stream, config, commonWindow) {
                    ...
                    if (this.windowSequence === ICStream.EIGHT_SHORT_SEQUENCE) {
                        ...
                        if (this.predictorPresent) this.decodePrediction(stream, config, commonWindow);
                    }
                },

                decodePrediction: function decodePrediction(stream, config, commonWindow) {
        ...
        var SWB_OFFSET_1024_96 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 64, 72, 80, 88, 96, 108, 120, 132, 144, 156, 172, 188, 212, 240, 276, 320, 384, 448, 512, 576, 640, 704, 768, 832, 896, 960, 1024]);

        var SWB_OFFSET_128_96 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 32, 40, 48, 64, 92, 128]);

        var SWB_OFFSET_1024_64 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 64, 72, 80, 88, 100, 112, 124, 140, 156, 172, 192, 216, 240, 268, 304, 344, 384, 424, 464, 504, 544, 584, 624, 664, 704, 744, 784, 824, 864, 904, 944, 984, 1024]);

        var SWB_OFFSET_128_64 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 32, 40, 48, 64, 92, 128]);

        var SWB_OFFSET_1024_48 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 80, 88, 96, 108, 120, 132, 144, 160, 176, 196, 216, 240, 264, 292, 320, 352, 384, 416, 448, 480, 512, 544, 576, 608, 640, 672, 704, 736, 768, 800, 832, 864, 896, 928, 1024]);

        var SWB_OFFSET_128_48 = new Uint16Array([0, 4, 8, 12, 16, 20, 28, 36, 44, 56, 68, 80, 96, 112, 128]);

        var SWB_OFFSET_1024_32 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 80, 88, 96, 108, 120, 132, 144, 160, 176, 196, 216, 240, 264, 292, 320, 352, 384, 416, 448, 480, 512, 544, 576, 608, 640, 672, 704, 736, 768, 800, 832, 864, 896, 928, 960, 992, 1024]);

        var SWB_OFFSET_1024_24 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 52, 60, 68, 76, 84, 92, 100, 108, 116, 124, 136, 148, 160, 172, 188, 204, 220, 240, 260, 284, 308, 336, 364, 396, 432, 468, 508, 552, 600, 652, 704, 768, 832, 896, 960, 1024]);

        var SWB_OFFSET_128_24 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 36, 44, 52, 64, 76, 92, 108, 128]);

        var SWB_OFFSET_1024_16 = new Uint16Array([0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 100, 112, 124, 136, 148, 160, 172, 184, 196, 212, 228, 244, 260, 280, 300, 320, 344, 368, 396, 424, 456, 492, 532, 572, 616, 664, 716, 772, 832, 896, 960, 1024]);

        var SWB_OFFSET_128_16 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 32, 40, 48, 60, 72, 88, 108, 128]);

        var SWB_OFFSET_1024_8 = new Uint16Array([0, 12, 24, 36, 48, 60, 72, 84, 96, 108, 120, 132, 144, 156, 172, 188, 204, 220, 236, 252, 268, 288, 308, 328, 348, 372, 396, 420, 448, 476, 508, 544, 580, 620, 664, 712, 764, 820, 880, 944, 1024]);

        var SWB_OFFSET_128_8 = new Uint16Array([0, 4, 8, 12, 16, 20, 24, 28, 36, 44, 52, 60, 72, 88, 108, 128]);

        var SWB_OFFSET_1024 = [SWB_OFFSET_1024_96, SWB_OFFSET_1024_96, SWB_OFFSET_1024_64, SWB_OFFSET_1024_48, SWB_OFFSET_1024_48, SWB_OFFSET_1024_32, SWB_OFFSET_1024_24, SWB_OFFSET_1024_24, SWB_OFFSET_1024_16, SWB_OFFSET_1024_16, SWB_OFFSET_1024_16, SWB_OFFSET_1024_8];

        var SWB_OFFSET_128 = [SWB_OFFSET_128_96, SWB_OFFSET_128_96, SWB_OFFSET_128_64, SWB_OFFSET_128_48, SWB_OFFSET_128_48, SWB_OFFSET_128_48, SWB_OFFSET_128_24, SWB_OFFSET_128_24, SWB_OFFSET_128_16, SWB_OFFSET_128_16, SWB_OFFSET_128_16, SWB_OFFSET_128_8];

        var SWB_SHORT_WINDOW_COUNT = new Uint8Array([12, 12, 12, 14, 14, 14, 15, 15, 15, 15, 15, 15]);

        var SWB_LONG_WINDOW_COUNT = new Uint8Array([41, 41, 47, 49, 49, 51, 47, 47, 43, 43, 43, 40]);
        ...
        var SAMPLE_RATES = new Int32Array([96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350]);
                ...
                for (var w = 0; w < windowCount; w++) {
                    if (this.nFilt[w] = stream.read(bits[0])) {
                        ...
                        for (var filt = 0; filt < nFilt_w; filt++) {
                            ...
                            if ((order_w[filt] = stream.read(bits[2])) > 20) throw new Error("TNS filter out of range: " + order_w[filt]);
